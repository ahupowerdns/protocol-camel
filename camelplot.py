...
data=pandas.read_csv("/home/ahu/git/dns-camel/camel.csv", delim_whitespace=True)
...
plt.stackplot(data["TIME"], 
              ...
              data["OBSOLETED"], data["UNKNOWN"],
             labels=["Internet Standard", "Proposed Standard", "Experimental", "Historic", "Best Current Practice", "Obsoleted", "Unknown"])
plt.legend(loc=2)
plt.grid()
plt.xlabel("Year")
plt.ylabel("Pages")
plt.title("Number of RFC pages covering DNS over time")
plt.savefig("camel-plot.png", dpi=300)
...
plt.ylabel("Pages")
plt.title("Number of RFC pages covering DNS over time")
plt.savefig("camel-plot-clean.png", dpi=300)
